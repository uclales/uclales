...
!
!       maxiter = max(nr(1:oldcell%nsplitters)/(10*minparentel),5)
! !       print *, maxiter
...
! !       print *, "TOTNEWCELLS",totnewcells, nrel_max
...
!         allocate(newcells(n)%p%siblings(newcells(n)%p%nsiblings))
!         nnn = 0
!         do nn = 1, totnewcells
!           if (n /= nn) then
!             nnn = nnn + 1
...
!       do n = 1,cell%nsiblings
!         ncells = ncells + 1
!         cell%siblings(n)%p%id = ncells
!       end do
...
!     do
!       if (iret == -1) exit
! !       print *, cell%nelements, minparentel
...
!  cbstep = (3000.)/distrange  !AXEL
  ...
  call check ( nf90_create(trim(stem)//'instant.nc', NF90_HDF5, fid))
  if (lthermal) then
