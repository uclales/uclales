...
Program bstate
  ...
  print *, "Enter case: CBL (1), RF02 (2), RF01 (default, or 0), ASTEX (3)"
  read  *, icase
  select case(icase)
  case(1)
...
contains
  ...
  subroutine gcss8
    ...
    real, parameter :: z_bl = 662.5
    real, parameter :: z_inv_top= 712.5
    real, parameter :: dz= 20.
    real, parameter :: psf= 1029.
    real, parameter :: thl_bl= 288.
    real, parameter :: qt_bl= 10.2
    real, parameter :: u_bl= -0.7
    real, parameter :: v_bl= -10.
    real, parameter :: u_fa= -2.
    real, parameter :: del_thl=5.5
    real, parameter :: del_qt=-1.1
    real, parameter :: thl_fa_grad=6e-3
    real, parameter :: qt_fa_grad=-2.8e-3
    real    :: zz, qq, uu, vv, thl, test
    integer    :: itest
    integer,parameter    :: nns=4
    real,dimension(nns) ::tts

    open (10,file='sound_in',status='unknown')
    write (10,'(5F10.3)') psf, thl_bl, qt_bl, u_bl, v_bl
    write (10,'(5F10.3)') z_bl, thl_bl, qt_bl, u_bl, v_bl
    zz = 675.
    do while (zz < 3200.)
!    do while (zz <= 2865.)
     if (zz <= z_inv_top) then   
       uu = u_bl-0.026*(zz-z_bl)
       vv = v_bl 
       qq = qt_bl-0.022*(zz-z_bl)
       thl= thl_bl+0.11*(zz-z_bl)
      else
       uu = u_fa
       vv = v_bl 
       qq = qt_bl+del_qt+qt_fa_grad*(zz-z_inv_top)
       thl= thl_bl+del_thl+thl_fa_grad*(zz-z_inv_top)
      end if    
       write (10,'(5F10.3)') zz, thl, qq, uu, vv
       zz = zz+dz
    end do
