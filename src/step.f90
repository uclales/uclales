...
!       if (istp==1) stop
  ...
  subroutine t_step
    ...
    use grid, only : level, dt, nstep, a_tt, a_up, a_vp, a_wp, dxi, dyi, dzi_t, &
         ...
         levset, a_rp, a_phi, a_alpha, a_beta, ls_cellkind, q0, q1, lstep, &!eckhard: needed for level set method
         dzi_m, qscr, euler                                                 !eckhard: needed for level set method
    use lset, only : ls_ghost,ls_advance,ls_finalize,ls_checkcons,ls_checkcells, &
         ls_nonconservative_fix
  ...
  subroutine update(nstep)
    ...
    use grid, only : a_xp, a_xt1, a_xt2, a_up, a_vp, a_wp, a_sp, dzi_t, dt,  &
         ...
         a_phi, a_phit1, a_phit2, levset, a_alpha, a_beta , lstep, &
         ls_cellkind, euler
    ...
    if (levset >= 1) then
        ...
        call phiset(nzp,nxp,nyp,a_phi,dzi_t) 
    end if
  ...
  subroutine boyanc(n1,n2,n3,level,wt,th,rt,rv,th00,scr)
    ...
    use defs, only: g, ep2
    use grid, only: a_rp, ls_q0p, ls_q1p, a_alpha, ls_buoy, levset
    use lset, only: bfunc
    ...
    do j=3,n3-2
       do i=3,n2-2
          ...
          if ((prescribed==1).and.(levset>=1)) then !overwrite buoyancy
             do k=1,n1
