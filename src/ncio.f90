...
contains
  ...
  subroutine write_anal(time)
    ...
    if (outtend)  then
         nn = nn+1
         iret = nf90_inq_varid(ncid0, sanal(nn), VarID)
         iret = nf90_put_var(ncid0, VarID, wtendt(:,i1:i2,j1:j2), start=ibeg, &
              count=icnt)
         nn = nn+1
         iret = nf90_inq_varid(ncid0, sanal(nn), VarID)
         iret = nf90_put_var(ncid0, VarID, wtendr(:,i1:i2,j1:j2), start=ibeg, &
              count=icnt)
         nn = nn+1
         iret = nf90_inq_varid(ncid0, sanal(nn), VarID)
         iret = nf90_put_var(ncid0, VarID, sgtendt(:,i1:i2,j1:j2)+adtendt(:,i1:i2,j1:j2), start=ibeg, &
              count=icnt)
         nn = nn+1
         iret = nf90_inq_varid(ncid0, sanal(nn), VarID)
         iret = nf90_put_var(ncid0, VarID, sgtendr(:,i1:i2,j1:j2)+adtendr(:,i1:i2,j1:j2), start=ibeg, &
              count=icnt)
         nn = nn+1
         iret = nf90_inq_varid(ncid0, sanal(nn), VarID)
         iret = nf90_put_var(ncid0, VarID, prect(:,i1:i2,j1:j2), start=ibeg, &
              count=icnt)
         nn = nn+1
         iret = nf90_inq_varid(ncid0, sanal(nn), VarID)
...
!     end if
  ...
  character (len=80) function ncinfo(itype,short_name)
    ...
    case('dqdt')    
       ...
       if (itype==2) ncinfo = 'tttt'
    case('prect')    
       if (itype==0) ncinfo = 'precip tnd of theta_l'
       if (itype==1) ncinfo = 'K/s'
       if (itype==2) ncinfo = 'tttt'
    case('precr')    
