...
!  use step, only : case_name
  ...
  integer :: fdmthd = 2          ! eckhard: 
  integer :: rimthd = 2          ! eckhard:
  integer :: prescrbuoy = 0      ! eckhard: compute buoyancy based on thermodynamics (0) or prescribed function (1)
  integer :: diffsclrs  = 1      ! eckhard: compute buoyancy based on thermodynamics (0) or prescribed function (1)
  integer :: entmodl = 0         ! eckhard: entrainment model for level set method

  integer :: gkmin = 0, gkmax = 0
  ...
  real              :: dxi, dyi, dt, psrf, MPI_partition_area
  ...
  real, dimension (:,:,:),   allocatable :: a_alpha,a_alphatilde,ls_absdphi, ls_nx, &
  ...
  integer, dimension (:,:),   allocatable :: ls_G,ls_Gtilde,ls_Gp,ls_Gpc,ls_Gm,ls_B,ls_dB,ls_UCN,ls_CN ! UCN...nodes of uncut cells
  integer, dimension (:,:),   allocatable :: ls_G_currently
  integer, allocatable :: ls_nG,ls_nGtilde,ls_nGp,ls_nGpc,ls_nGm,ls_nB,ls_ndB,ls_nUCN,ls_nCN ! number of cells in the subsets G, B, U, and dB
  integer, allocatable :: ls_nG_currently 
  !
...
contains
 ...
 !
    ...
    if (levset >= 1) then
        ...
        allocate( a_alphatilde(nzp,nxp,nyp) )
  ...
  subroutine define_grid
    ...
    MPI_partition_area = (xm(nxp-2) - xm(2)) * (ym(nyp-2) - ym(2))
    !      
...
!irina
    ...
    if (levset == 2) then
      ...
      nvar0 = nvar0+1
      sanal(nvar0) = sbase(nbase+21) !celltype
    end if
  ...
  subroutine write_anal(time)
    ...
    iret = nf90_put_var(ncid0, VarID, th00 + a_tp(:,i1:i2,j1:j2), start=ibeg, &
         count=icnt)
  ...
  !irina          
    ...
    if (levset == 2) then
       ...
       iret = nf90_put_var(ncid0, VarID, ls_q0(:,i1:i2,j1:j2,4), start=ibeg, &
            count=icnt)
       nn = nn+1
       iret = nf90_inq_varid(ncid0, 'q1', VarID)
       iret = nf90_put_var(ncid0, VarID, ls_q1(:,i1:i2,j1:j2,4), start=ibeg, &
            count=icnt)
       nn = nn+1
       ...
       iret = nf90_put_var(ncid0, VarID, ls_buoy(:,i1:i2,j1:j2), start=ibeg, &
            count=icnt)
       nn = nn+1
       iret = nf90_inq_varid(ncid0, 'cellkin', VarID)
       iret = nf90_put_var(ncid0, VarID, ls_cellkind(:,i1:i2,j1:j2), start=ibeg, &
            count=icnt)
    end if
