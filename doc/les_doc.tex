...
\baselineskip7mm
\bigskip
\author{\LARGE Bjorn Stevens \\ \footnotesize \it updated by Malte Rieck} 
\maketitle
...
are denoted by an overbar. Roughly speaking they can be thoughtof as
...
denotes the deviation of $\bar \theta_v$ from its horizontal average
(rather than from the basic-state).  This ensures that no mean
vertical accelerations arise.  For consistency this requires the
...
the desity weighted divergence of (\ref{eq:FM}), yields $\tilde{\pi}$ through the
...
The radiation type can be set by the variable iradtype in the
NAMELIST file. As a default, calculations are done without
radiation (iradtype=1). One has the option of using either
the simple radiation (iradtype=2) of gcss intercomparison or
the full radiation (iradtype=4) of Prescas and Stevens.
Here, for simple radiation treatment, short- and longwave
radiation will be computed as described in \emph{src/rad\_gcss.f90}.
The full radiation approach makes use of \emph{bin/datafiles}
to apply radiation values above 3km. A particular treatment
of radiation is available for the smoke cloud and the bellon
case. The radiation treatment for the smoke cloud case can be
used by setting iradtype=2 and level=1. The bellon case has a
specific radiation code as shown in the bellon\_rad routine
and can be used by setting iradtype=3.
...
where the number of grid points is evenly devided over the processors.
...
\\
\\
2D paralellization allows us in principle to use $N_xN_y/4$
...
The UCLALES Code is free for all to use, distribute, and call their own,
...
Subsequent developments of the code are available to its community here as well.
The newest version of UCLALES will always be available at \textbf{http://git.gitorious.org/uclales}.
To Download the newest UCLALES version make sure you have git installed on your system.
...
``git status'' and ``git diff''. You can also insert your private UCLALES version into git.
...
level   & '' & 0 & 0=thermodynamic level, 1=dry cbl, 2=moist cbl (no rain), 3=moist cbl (with rain)\\
\hline
...
timmax, iradtype,level and others before running the model.
...
quantity can be difficult (i.e. conditional avarages).
...
L\"upkes, C., K.~D. Beheng, and G.~Doms, 1989: A paraemeterization scheme
